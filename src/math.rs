...
/// Get the smaller signed angle from 'souce' to 'target'.
...
impl<T> WeightedMean for T where 
    T: AddAssign + Mul<f64, Output = T> + Div<f64, Output = T> + Copy + Default
{
    fn weighted_mean(it: impl Iterator<Item = (Self, f64)>) -> Option<Self> {
